                    ...
                    <div class="row">
                        <div class="col-md-12 reports clearfix">
                            ...
                            <div class="form-group col-md-3">
                                ...
                                <select name="locality_id" id="locality" class="chzn-select">
                                    <option value=""><?php echo get_languageword('select_an_option'); ?></option>
                                    <!-- Options will be populated here by the AJAX call -->
                                </select>
                            </div>

                            <div class="form-group col-md-3">
                                <button type="submit" name="location_wise_reports" value="location_wise_reports"
                                    class="btn btn-primary btn-lg"><?php echo get_languageword('submit'); ?></button>
                            </div>
                        ...
                        <table id="example" class="display responsive nowrap" width="100%" cellspacing="0">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td><?php if (isset($r->order_date)) echo get_date($r->order_date) . ' - ' . $r->order_time; ?>
                                    </td>
                                    <td><?php if (isset($r->payment_type)) echo $r->payment_type; ?><?php if ($r->payment_gateway != '') echo ' - ' . $r->payment_gateway; ?>
                                    </td>
...
function get_localities(city_id) {
    console.log("Selected City ID:", city_id); // Debugging line
    $("#locality").empty();
    ...
    if (city_id > 0) {
        $.ajax({
            ...
            success: function(data) {
                console.log("AJAX Response Data:", data); // Debugging line
                if (data != '' && data != 0) {
                    $('#locality').empty();
...
$(document).ready(function() {
    <?php if (isset($_POST['location_wise_reports'])) { ?>
    var city_id = $("#city_id option:selected").val();
    console.log("Page Loaded, Selected City ID:", city_id); // Debugging line
    get_localities(city_id);
    <?php } ?>
});
</script>
